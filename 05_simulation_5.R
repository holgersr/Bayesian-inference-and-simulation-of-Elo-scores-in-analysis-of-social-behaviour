...
for(rep_index in 1:repeats){
  ...
  plot(1:4000, sum_abs_error/length(truth), type = "s", bty = "n", 
...
dev.off()
##
# pdf("simulation5a.pdf", height = 5, width = 7)
# par(mfrow = c(1, 2), mar = c(4, 4, 1, 1))
# elo_sim_list <- vector("list", repeats)
# for(rep_index in 1:repeats){
#   X_sim <- X_sim_list[[rep_index]]
#   bayes_starting_scores <- summary(m_list[[rep_index]])$summary[1:ncol(X_sim)]
#   bayes_k <- summary(m_list[[rep_index]])$summary[ncol(X_sim)+1]
#   elo_sim_list[[rep_index]] <- Elo_pA(EloStart_logk = c(bayes_starting_scores, log(bayes_k)), 
#                                       X = X_sim, presence = 1 + 0*X_sim)$Elo
#   ylim <- range(c(as.numeric(elo_sim_list[[rep_index]]), starting_scores1, starting_scores2))
#   elo_sim <- elo_sim_list[[rep_index]]
#   plot(elo_sim[, 1], type = "n", ylim = ylim, bty = "n", yaxt = "n", 
#        xlab = "Interaction index", ylab = "Elo score")
#   axis(2, lab = FALSE, at = seq(-5, 5, by = 5))
#   mtext(side = 2, at = seq(-5, 5, by = 5), text = seq(-5, 5, by = 5), cex = 1, 
#         line = 0.7, las = 2)
#   points(rep(1, length(starting_scores1)), starting_scores1, col = cols, pch = 16)
#   points(rep(period_length, length(starting_scores2)), starting_scores2, col = cols, pch = 16)
#   for(i in 1:ncol(X_sim)){
#     lines(1:period_length, elo_sim[1:period_length, i], 
#           col = cols[i], lty = 1, lwd = 1)
#     lines(period_length + (1:period_length), elo_sim[period_length + (1:period_length), i], 
#           col = cols[i], lty = 1, lwd = 1)
#   }
# }
# dev.off()